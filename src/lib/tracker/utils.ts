...
export interface TieredTracker {
  tier: number;
  trackers: string[];
}

const knownPublicTrackers: string[] = [
...
export function groupTrackers(trackers: string[][]): TieredTracker[] {
  return trackers.map((tierTrackers, index) => ({
    tier: index,
    trackers: tierTrackers
  }));
}

export function isPublicTracker(url: string): boolean {
  return knownPublicTrackers.includes(url);
}